    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->portNameComboBox->addItem( "tty0");
    ui->portNameComboBox->addItem( "tty1");
    ui->portNameComboBox->addItem( "tty2");
    ui->portNameComboBox->addItem( "tty3");
    ui->portNameComboBox->addItem( "tty4");
    ui->portNameComboBox->addItem( "tty5");
    ui->portNameComboBox->addItem( "tty8");
...
void MainWindow::HandlePacket(Packet &packet)
{
    QByteArray mac;
    QString str;
    switch(packet.type)
    {
    case 0xF0:
                str=QString("%1%2%3%4%5%6 号节点返回心跳信号").arg((int)packet.src_mac[0]).arg((int)packet.src_mac[1])
                         .arg((int)packet.src_mac[2]).arg((int)packet.src_mac[3]).arg((int)packet.src_mac[4]).arg((int)packet.src_mac[5]);
             break;
    case 0xF1:
             qDebug()<<packet.type;
             qDebug()<<packet.content_len;

             for(int i=0;i<6;i++){
                 mac.append((int)packet.src_mac[i]);

             }
             str=QString("%1%2%3%4%5%6 号节点请求入网").arg((int)packet.src_mac[0]).arg((int)packet.src_mac[1])
                     .arg((int)packet.src_mac[2]).arg((int)packet.src_mac[3]).arg((int)packet.src_mac[4]).arg((int)packet.src_mac[5]);

             qDebug()<<str;
             qDebug()<<packet.src_mac[0];
             qDebug()<<packet.des_mac[0];
             qDebug()<<packet.net_id;
             qDebug()<<packet.head_check;
             qDebug()<<packet.content;
             qDebug()<<packet.content_check;
             break;
    case 0xF2:
              str=QString("%1%2%3%4%5%6 号节点入网成功").arg((int)packet.src_mac[0]).arg((int)packet.src_mac[1])
                      .arg((int)packet.src_mac[2]).arg((int)packet.src_mac[3]).arg((int)packet.src_mac[4]).arg((int)packet.src_mac[5]);
...
void MainWindow::readMyCom()
{
    ...
    if(!temp.isEmpty()){
            ...
            if(buf=="74 69 6D 65 5F 6F 75 74 21 0D 0A"){
                qDebug("time_out@^@");
                readBuffer.resize(0);
            }
...
//        }
        ...
        while(readSize>2){
           char *s=readBuffer.data();
            ...
            pkt.type=s[0];
            pkt.content_len=s[1];
            memcpy(pkt.src_mac,s+2,6);
            memcpy(pkt.des_mac,s+8,6);
            pkt.net_id=(((ushort)s[14])<<8)|(((ushort)s[15])&0x00FF);
            pkt.head_check=(((ushort)s[20])<<8)|(((ushort)s[21])&0x00FF);
            memcpy(pkt.content,s+22,pkt.content_len);
            pkt.content_check=(((ushort)s[22+pkt.content_len])<<8)|(((ushort)s[23+pkt.content_len])&0x00FF);
            HandlePacket(pkt);

            readSize -=PacketSize;
            QByteArray newBuffer((char *)(s+PacketSize+1),readSize);
